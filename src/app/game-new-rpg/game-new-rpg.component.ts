  ...
  runAnimate() {
    ...
    if (this.doors !== undefined) {
      this.doors.forEach((door: any) => {
        if (
          this.player.object.position.distanceTo(door.trigger.position) < 100
        ) {
    ...
    if (this.collect !== undefined && !trigger) {
      this.collect.forEach((object: any) => {
        if (
          object.visible &&
          this.player.object.position.distanceTo(object.position) < 100
        ) {
          this.actionBtn.style = 'display:block;';
          this.onAction = {
    ...
    loader.load('assets/lotus/fbx/girl-walk.fbx', (object: any) => {
      ...
      object.mixer.addEventListener('finished', (e: any) => {
        ...
        if (this.player.cameras.active == this.player.cameras.collect) {
          console.log(
            'this.player.cameras.active == this.player.cameras.collect'
          );
    ...
    if (this.onAction.mode !== undefined) {
      switch (this.onAction.mode) {
        case 'open-doors':
          ...
          this.tweens.push(
            new Tween(left.position, 'z', left.position.z - 240, 2, () => {
              game.tweens.splice(game.tweens.indexOf(this), 1);
            })
          );
          this.tweens.push(
            new Tween(right.position, 'z', right.position.z + 240, 2, () => {
              game.tweens.splice(game.tweens.indexOf(this), 1);
        ...
        case 'collect':
          ...
          if (
            briefcase &&
            briefcase.children[0] &&
            briefcase.children[0].children[0] &&
            briefcase.children[0].children[0].children[this.onAction.index] &&
            briefcase.children[0].children[0].children[this.onAction.index]
              .children[0]
          ) {
            (
